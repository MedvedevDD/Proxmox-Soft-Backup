...
def recovery_doctor():
    ...
    add('nut systemd directory', Path('/etc/systemd/system/nut-driver@ippon.service.d').is_dir(), '/etc/systemd/system/nut-driver@ippon.service.d')
    ...
    return {'area':'recovery','ready':overall,'checks':checks,'supported_execute_targets':['grafana/database','grafana/configuration','grafana/plugins','influxdb/database','influxdb/configuration','telegraf/configuration','telegraf/state','nut/configuration','nut/state','nut/systemd']}
    ...
    if args.command=='doctor':
        if args.area=='recovery':
            ...
            if args.json: print(json.dumps(report,indent=2,sort_keys=True))
                ...
                print('Supported execute targets: grafana/database, grafana/configuration, grafana/plugins, influxdb/database, influxdb/configuration, telegraf/configuration, telegraf/state, nut/configuration, nut/state, nut/systemd')
    ...
    if args.command=='recover':
        if args.execute:
            ...
            if (args.application,args.component) not in supported:
                print('ERROR: execute mode supports only grafana/database, grafana/configuration, grafana/plugins, influxdb/database, influxdb/configuration, telegraf/configuration, telegraf/state, nut/configuration, nut/state, and nut/systemd',file=sys.stderr); return 2
            if not args.rollback_destination:
            ...
            try:
                executors={
                    ...
                    ("nut","systemd"): execute_nut_systemd_recovery,
                }
