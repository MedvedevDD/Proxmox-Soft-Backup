...
def recovery_doctor():
    ...
    add('grafana plugins directory', Path('/var/lib/grafana/plugins').is_dir(), '/var/lib/grafana/plugins')
    ...
    add('nut state directory', Path('/var/lib/nut').is_dir(), '/var/lib/nut')
    ...
    return {'area':'recovery','ready':overall,'checks':checks,'supported_execute_targets':['grafana/database','grafana/configuration','grafana/plugins','influxdb/database','influxdb/configuration','telegraf/configuration','telegraf/state','nut/configuration','nut/state']}
...
def main():
    ...
    if args.command=='doctor':
        if args.area=='recovery':
            ...
            if args.json: print(json.dumps(report,indent=2,sort_keys=True))
                ...
                print('Supported execute targets: grafana/database, grafana/configuration, grafana/plugins, influxdb/database, influxdb/configuration, telegraf/configuration, telegraf/state, nut/configuration, nut/state')
    ...
    if args.command=='recover':
        if args.execute:
            supported={("grafana","database"),("grafana","configuration"),("grafana","plugins"),("influxdb","database"),("influxdb","configuration"),("telegraf","configuration"),("telegraf","state"),("nut","configuration"),("nut","state")}
            if (args.application,args.component) not in supported:
                print('ERROR: execute mode supports only grafana/database, grafana/configuration, grafana/plugins, influxdb/database, influxdb/configuration, telegraf/configuration, telegraf/state, nut/configuration, and nut/state',file=sys.stderr); return 2
            ...
            try:
                executors={
                    ...
                    ("grafana","configuration"): execute_grafana_configuration_recovery,
                    ("grafana","plugins"): execute_grafana_plugins_recovery,
                    ...
                    ("nut","state"): execute_nut_state_recovery,
                }
